...
export const SWIPE_THRESHOLD = 15; // Further reduced for even more responsive swipes
...
export const swipePower = (offset: number, velocity: number): number => {
  ...
  return Math.abs(offset) * Math.max(1.8, velocity); // Added minimum velocity factor
};
